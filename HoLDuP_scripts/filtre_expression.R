...
n_cell_lines=(length(args)-2)/3

threshold_read_at_least_once=as.double(args[1])
threshold_rpkm_replicate=as.double(args[2])

count_file_paths=args[(1:n_cell_lines)+2]
group_cell_lines=args[(1:n_cell_lines)+n_cell_lines+2]
rpkm_file_paths=args[(1:n_cell_lines)+2*n_cell_lines+2]
